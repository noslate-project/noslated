...
export class CapacityManager extends Base {
  ...
  async autoScale() {
    // 先创建 brokers
    const { reservationCountPerFunction } =
      this.plane.capacityManager.config.worker;
    for (const profile of this.plane.capacityManager.functionProfileManager
      .profile) {
      ...
      if (reservationCount || reservationCountPerFunction) {
        this.plane.capacityManager.workerStatsSnapshot.getOrCreateBroker(
          profile.name,
          false,
    ...
    const brokers = [
      ...this.plane.capacityManager.workerStatsSnapshot.brokers.values(),
    ];
    const { expandDeltas, shrinkDeltas } = this.evaluteScaleDeltas(brokers);
  ...
  get virtualMemoryUsed() {
    return [...this.workerStatsSnapshot.brokers.values()].reduce(
      (memo, broker) => memo + broker.virtualMemory,
      0
    );
  }

  async updateWorkerContainerStatus(
    ...
    if (broker && worker) {
      ...
      if (
        ...
        this.logger.info(
          'will kill worker(%s) with request(%s) because useCGIMode=true and request drained.',
          worker.name,
          report.requestId
        );

        this.plane.controller
          .stopWorker(worker.name, report.requestId)
          .then(() => {
            this.logger.info(
      ...
      this.logger.warn(
        'updateWorkerContainerStatus report [%o] skip because no broker and worker related.',
        report
      );
    }
  }
   ...
   */
  async expandDueToQueueingRequest(
    client: DataPlaneClient,
    request: NotNullableInterface<root.noslated.data.IRequestQueueingBroadcast>
  ) {
