...
describe(common.testName(__filename), function () {
  ...
  describe('#ready()', () => {
    it('should start', async () => {
      manager = new Manager(
        {
          dirs: { noslatedSock: addressDirname },
    ...
    it('should emit newClientReady', async () => {
      ...
      await manager.ready();
      clients.sort((a, b) => {
        return a.planeId < b.planeId ? -1 : 1;
      });
  ...
  describe('#availableClients()', () => {
    ...
    it('should return only one client', async () => {
      ...
      assert.strictEqual(
        manager.availableClients().length,
        availableClientsLength - 1
      );
    });
  });
  ...
  describe('_onClientReady()', () => {
    it('should call _onClientReady() after client reconnected', async () => {
      ...
      manager
        .clients()
        .forEach((client: BasePlaneClient & { readyCount?: number }) => {
  ...
  describe('#callToAllAvailableClients', () => {
    it('should call all clients', async () => {
      ...
      const ret = await manager.callToAllAvailableClients(
        'ping',
        [{ msg: 'hello' }],
        'all'
      );
    ...
    it('should call all clients (allSettled)', async () => {
      ...
      assert.deepStrictEqual(ret, [
        { value: { msg: 'hello' }, status: 'fulfilled' },
      ]);
    });
    ...
    it('should call all clients (all with error)', async () => {
      ...
      await assert.rejects(
        manager.callToAllAvailableClients('ping', [{ msg: 'hello' }], 'all'),
        {
