...
class Worker {
  ...
  /**
   * Sync data from data panel and turf.
  ...
  switchTo(found: TurfProcess | null, timestamp: number) {
    ...
    switch (turfState) {
      ...
      case TurfContainerStates.running: {
        if (Date.now() - this.#registerTime > this.#config.worker.controlPanelConnectTimeout && this.#containerStatus === ContainerStatus.Created) {
