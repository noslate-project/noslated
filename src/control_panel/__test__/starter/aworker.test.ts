...
import { Aworker } from '#self/control_panel/starter/index';
import { turf, startTurfD, stopTurfD } from '#self/lib/turf';
import * as testUtil from '#self/test/util';
import { ControlPanel } from '#self/control_panel/control_panel';
...
describe(common.testName(__filename), () => {
  ...
  conditionalDescribe('#constructor()', () => {
    it('should start seed', async () => {
      let aworker;
      try {
        aworker = new Aworker(dummyPanel as any, config);
        await aworker.ready();
  ...
  conditionalDescribe('#keepSeedAliveTimer', () => {
    it('seed should keep alive', async function() {
      ...
      const aworker = new Aworker(dummyPanel as unknown as ControlPanel, config);
      await aworker.ready();
      await aworker.waitSeedReady();
  ...
  conditionalDescribe('#start()', () => {
    ...
    it('should start without seed', async () => {
      ...
      try {
        ...
        aworker = new Aworker(dummyPanel as ControlPanel, config);
        await aworker.ready();
