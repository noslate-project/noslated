...
import { DataPanelClientManager } from '#self/control_panel/data_panel_client/manager';
import { mockClientCreatorForManager } from '#self/test/util';
import * as starters from '#self/control_panel/starter/index';
import { turf } from '#self/lib/turf';
import { CapacityManager } from '#self/control_panel/capacity_manager';
...
describe(common.testName(__filename), () => {
  ...
  /**
   * clock mock 及还原顺序会导致 control_panel 无法关闭
   */
  beforeEach(async () => {
  ...
  describe('#tryExpansion()', () => {
    it('should try expansion', async () => {
      let called = 0;
      mm(capacityManager.panel.workerLauncher, 'tryLaunch', async (name: any, options: any) => {
      ...
      called = 0;
      mm(capacityManager.panel.workerLauncher, 'tryLaunch', async () => {
        called++;
  ...
  describe('#autoScale()', () => {
    for (let id = 0; id < 2; id++) {
      it(`should auto scale with ${id === 0 ? 'enough' : 'not enough'} memory`, async () => {
        ...
        mm(capacityManager.panel.dataPanelClientManager, 'reduceCapacity', async (data: { brokers: string | any[]; }) => {
    ...
    it('a wrong situation of worker count infinitely increasing', async () => {
      ...
      //                 ...
      //                 at async CapacityManager.#expand (/usr/local/noslate/control_panel/capacity_manager.js:111:5)
      //                 at async CapacityManager.autoScale (/usr/local/noslate/control_panel/capacity_manager.js:200:7)
