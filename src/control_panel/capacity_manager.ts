...
import { DataPanelClient } from './data_panel_client/client';
...
export class CapacityManager extends Base {
  ...
  constructor(public panel: ControlPanel, private config: Config) {
    super();

    this.functionProfileManager = panel.functionProfile;
    this.workerStatsSnapshot = new WorkerStatsSnapshot(panel.functionProfile, config);
  ...
  async tryExpansion(
    ...
    const { workerLauncher } = this.panel;
    const ret = [];
  ...
  async #expand(deltas: number[], brokers: Broker[]) {
    ...
    for (let i = 0; i < deltas.length; i++) {
      if (deltas[i] > 0) {
        const profile = this.panel.functionProfile.get(brokers[i].name);
  ...
  async #doShrink(deltas: number[], brokers: Broker[]) {
    ...
    const { dataPanelClientManager } = this.panel;
    const ensured = await dataPanelClientManager.reduceCapacity({ brokers: shrinkData });
    ...
    this.logger.info('Up to do shrink destroy after asking for data panel.', up);
   ...
   * @param {{ data: import('#self/lib/proto/alice/data-panel-broadcast').RequestQueueingBroadcast, client: import('#self/control_panel/data_panel_client/client') }} task The task object.
   */
  async expandDueToQueueingRequest(client: DataPanelClient, request: NotNullableInterface<root.alice.data.IRequestQueueingBroadcast>) {
    ...
    const profile = this.panel.functionProfile.get(name);

    try {
      this.logger.info('Prepare to launch worker(%s, inspect %s)', name, isInspect);
      const now = performance.now();

      await this.panel.workerLauncher.tryLaunch(
        name,
        {
          inspect: isInspect,
        },
