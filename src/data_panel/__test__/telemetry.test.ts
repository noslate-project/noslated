...
describe(common.testName(__filename), function() {
  ...
  it('invoke function with metrics', async () => {
    ...
    {
      const records = getMetricRecords(processor!, DataPanelMetrics.INVOKE_COUNT, {
        [PanelMetricAttributes.FUNCTION_NAME]: nodeJsWorkerTestItem.name,
        [PanelMetricAttributes.SERVICE_NAME]: '',
      });
    ...
    {
      const records = getMetricRecords(processor!, DataPanelMetrics.INVOKE_DURATION, {
        [PanelMetricAttributes.FUNCTION_NAME]: nodeJsWorkerTestItem.name,
        [PanelMetricAttributes.SERVICE_NAME]: '',
      });
      assert.strictEqual(records.length, 1);
    ...
    {
      const records = getMetricRecords(processor!, DataPanelMetrics.QUEUED_REQUEST_COUNT, {
        [PanelMetricAttributes.FUNCTION_NAME]: nodeJsWorkerTestItem.name,
      });
      assert.strictEqual(records.length, 1);
      assert.strictEqual(records[0].aggregator.toPoint().value, 1);
    ...
    {
      const records = getMetricRecords(processor!, DataPanelMetrics.QUEUED_REQUEST_DURATION, {
        [PanelMetricAttributes.FUNCTION_NAME]: nodeJsWorkerTestItem.name,
      });
      assert.strictEqual(records.length, 1);
  ...
  it('invoke service with metrics', async () => {
    ...
    await forceExport(meterProvider!);
    {
      const records = getMetricRecords(processor!, DataPanelMetrics.INVOKE_COUNT, {
        [PanelMetricAttributes.FUNCTION_NAME]: nodeJsWorkerTestItem.name,
        [PanelMetricAttributes.SERVICE_NAME]: 'foobar',
      });
      assert.strictEqual(records.length, 1);
      assert.strictEqual(records[0].aggregator.toPoint().value, 1);
    }
