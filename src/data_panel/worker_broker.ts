...
import { PanelMetricAttributes } from '#self/lib/telemetry/semantic_conventions';
...
import { kMemoryLimit } from '#self/control_panel/constants';
...
export class WorkerBroker extends Base {
  ...
  tryConsumeQueue(notThatBusyWorker: Worker) {
    ...
    while (this.requestQueue.length && notThatBusyWorker.maxActivateRequests > notThatBusyWorker.activeRequestCount) {
      ...
      this.dataFlowController.queuedRequestDurationRecorder.add(Date.now() - request.startEpoch, {
        [PanelMetricAttributes.FUNCTION_NAME]: this.name,
      });
   ...
   * @param {import('#self/lib/proto/alice/data-panel').StartWorkerFastFailRequest} startWorkerFastFailRequest The fast fail request.
   */
