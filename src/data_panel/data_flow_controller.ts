...
import { getCurrentPanelId, setDifference } from '#self/lib/util';
...
import { DataPanelMetrics, PanelMetricAttributes } from '#self/lib/telemetry/semantic_conventions';
...
export class DataFlowController extends BaseOf(EventEmitter) {
  ...
  constructor(public host: Host, public config: Config) {
    ...
    this.meter = getMeter();
    this.#invokeCounter = this.meter.createCounter(DataPanelMetrics.INVOKE_COUNT, {});
    // TODO: histogram
    this.#invokeDurationRecorder = this.meter.createCounter(DataPanelMetrics.INVOKE_DURATION, {});

    this.queuedRequestCounter = this.meter.createCounter(DataPanelMetrics.QUEUED_REQUEST_COUNT, {});
    // TODO: histogram
    this.queuedRequestDurationRecorder = this.meter.createCounter(DataPanelMetrics.QUEUED_REQUEST_DURATION, {});

    const delegateSockPath = this.delegateSockPath = path.join(
      this.config.dirs.aliceSock,
      `dlgt-${getCurrentPanelId()}.sock`);
   ...
   * @param {import('#self/lib/proto/alice/data-panel').CapacityReductionBroker} broker The capacity reduction broker object in Protobuf.
   * @param {WorkerBroker} realBroker The real broker object with methods.
   * @param {import('#self/lib/proto/alice/data-panel').CapacityReductionBroker} toBeClosed The capacity reduction broker object in Protobuf with closed workers.
   ...
   * @param {import('#self/lib/proto/alice/data-panel').CapacityReductionBroker[]} workersInfo The brokers including workers information.
   * @return {Promise<import('#self/lib/proto/alice/data-panel').CapacityReductionBroker[]>} The brokers including workers that traffic closed.
   */
   ...
   * @type {import('#self/lib/proto/alice/data-panel-broadcast').BrokerStats[]} The brokers with workers' stats.
   */
  ...
  async invoke(name: string , inputStream: Buffer | Readable, metadata: Metadata, { serviceName = '' }: InvokeContext = {}): Promise<TriggerResponse> {
    ...
    try {
      ...
      this.#invokeCounter.add(1, {
        [PanelMetricAttributes.FUNCTION_NAME]: name,
        [PanelMetricAttributes.SERVICE_NAME]: serviceName,
      });
