...
export class WorkerTelemetry {
  ...
  constructor(
    ...
    this.#observables.set(
      ...
      this.#meter.createObservableGauge(WorkerMetrics.TOTAL_HEAP_SIZE)
    );
    this.#observables.set(
      WorkerMetrics.USED_HEAP_SIZE,
      this.#meter.createObservableGauge(WorkerMetrics.USED_HEAP_SIZE)
    );

    this.#meter.addBatchObservableCallback(
      this.onObservation,
      Array.from(this.#observables.values())
    );
  }

  onObservation = async (batchObservableResult: BatchObservableResult) => {
    await Promise.all(
      Array.from(this.#dataFlowController.brokers.values()).flatMap(broker => {
        ...
        return Array.from(broker.workers()).map(async worker => {
          ...
          try {
            ...
            logger.warn(
              `Failed to collect metrics for worker ${worker.name}.`,
              e
            );
            return;
          }
