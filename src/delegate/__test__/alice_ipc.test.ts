...
describe(testName(__filename), () => {
  describe('ipc', () => {
    ...
    it('should connect and request', async () => {
      ...
      server.onRequest = (sessionId, op, params, callback) => {
        ...
        if ((params as aworker.ipc.ICredentialsRequestMessage).cred !== 'foobar') {
    ...
    describe('AliceServer', () => {
      ...
      it('should trigger requests', async () => {
        ...
        const ret = server.trigger(sessionId, 'test', metadata, false, false, 1000);
      ...
      it('should trigger requests with server streams', async () => {
        ...
        const interval = setInterval(() => {
          ...
          server.streamPush(sessionId, streamId!, times === 0, Buffer.from('foobar'), false)
        }, 1);
      ...
      it('should handle requests in loop ticks', async () => {
        ...
        const events: string[] = [];
        server.onRequest = (sid, op, params, callback) => {
          events.push(`${(params as aworker.ipc.IDaprInvokeRequestMessage).methodName} on request`);
          Promise.resolve()
            .then(() => {
              events.push(`${(params as aworker.ipc.IDaprInvokeRequestMessage).methodName} microtask`);
  ...
  describe('MessageParser', () => {
    it('should handle buffer upper bounds', () => {
      ...
      const block = Buffer.concat([Buffer.alloc(1000), header, content, Buffer.alloc(1000)]);
      const view = Buffer.from(block.buffer, block.byteOffset + 1000, header.byteLength + content.byteLength);
      parser.push(view);
      ...
      assert.strictEqual(message.requestKind, aworker.ipc.RequestKind.StreamPush);
      assert.strictEqual(message.requestId, 1);
      assert.strictEqual(Buffer.from(aworker.ipc.StreamPushRequestMessage.encode(message.content).finish()).compare(content), 0);
